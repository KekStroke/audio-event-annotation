...
- Генерируется на лету без кэширования
...
A: Изображения генерируются на лету для каждого запроса и не сохраняются на диск. При повторном обращении создаётся новое изображение.
...
A: Кэширование не используется — все изображения строятся заново при каждом запросе.
