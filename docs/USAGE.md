...
```bash
GET /api/audio/{id}/waveform?width=1200&height=300
```

Параметры:
- `width` (опционально): Ширина изображения (по умолчанию 1200)
- `height` (опционально): Высота изображения (по умолчанию 300)
...
- `width` (опционально): Ширина изображения (по умолчанию 1024)
- `height` (опционально): Высота изображения (по умолчанию 512)
- `color_map` (опционально): Цветовая карта (по умолчанию "viridis")
...
# Получить waveform (генерируется на лету без кэширования)
