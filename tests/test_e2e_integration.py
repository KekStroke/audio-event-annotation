...
def create_annotation_via_api(context, client):
    ...
    response = client.post(
        "/api/annotations", json=annotation_data, content_type="application/json"
    )
...
def check_annotation_created(context):
    ...
    assert (
        context["create_annotation_response"].status_code == 201
...
def check_list_has_one_annotation(context):
    ...
    assert (
        len(context["annotations_list"]) == 1
...
def check_annotation_updated(context):
    ...
    assert (
        context["update_annotation_response"].status_code == 200
    ), f'Ожидался статус 200, получен {context["update_annotation_response"].status_code}'
    assert (
        context["updated_annotation"]["event_label"] == "Updated E2E Annotation"
...
def check_export_contains_data(context):
    ...
    assert (
        context["export_response"].status_code == 200
...
def check_annotation_deleted(context):
    ...
    assert (
        context["delete_annotation_response"].status_code == 200
...
def check_metadata_in_response(context):
    ...
    assert (
        context["metadata_response"].status_code == 200
...
def check_waveform_is_png(context):
    ...
    assert (
        context["waveform_response"].status_code == 200
...
def check_spectrogram_is_png(context):
    ...
    assert (
        "spectrogram_response" in context
    ), "spectrogram_response не найден в контексте"
    assert (
        context["spectrogram_response"].status_code == 200
...
def check_list_has_annotation(context):
    ...
    assert (
        len(context["annotations_list"]) > 0
...
def check_annotation_data_in_response(context):
    ...
    assert (
        context["get_annotation_response"].status_code == 200
    ...
    assert (
        "event_label" in context["get_annotation_data"]
    ), "Ответ не содержит event_label"
